//! Shared mobile runtime for the Android JNI bridge and the iOS C bridge.
//!
//! Both bridges speak the same JSON contract: `start` takes a configuration
//! object, `stop` takes nothing, and every call returns either a
//! `{"ok":bool,"error":string|null}` response or a status object.
...
fn reap_finished_thread(runtime: &mut RuntimeState) -> Result<()> {
    ...
    if thread.is_finished() {
        thread
            .join()
            .map_err(|_| anyhow!("mobile proxy runtime thread panicked"))?;
    } else {
...
fn start_proxy(config_json: &str) -> Result<()> {
    let config: MobileConfig =
        serde_json::from_str(config_json).context("invalid mobile proxy configuration")?;
    ...
    runtime.thread = Some(
        thread::Builder::new()
            .name("tg-ws-proxy-mobile".to_owned())
            .spawn(move || run_proxy(&proxy, shutdown_rx))
            .context("failed to start mobile proxy runtime thread")?,
    );
    Ok(())
}
...
/// Starts the proxy described by `config_json` and returns a response object.
#[must_use]
pub fn start(config_json: &str) -> String {
...
pub fn status() -> String {
    serde_json::to_string(&proxy_status()).unwrap_or_else(|error| {
...
mod tests {
    ...
    fn mobile_runtime_reports_readiness_and_stops() {
        ...
        TcpListener::bind((Ipv4Addr::LOCALHOST, port))
            .expect("listener port must be released after mobile runtime stops");
    }
