...
Версия `0.1.1` использует Rust-ядро `1.10.0`: direct WebSocket/CfProxy upload
передаёт splitter уже расшифрованный MTProto chunk вместе с Telegram
ciphertext и не выполняет повторный AES-CTR проход.

> [!WARNING]
>
> Android-сборка публикуется в стабильном канале, проходит CI, а для `0.1.0`
> выполнен emulator smoke-тест. Проверки Telegram, мобильной сети и
> энергосбережения на разных реальных телефонах всё ещё нужны.
