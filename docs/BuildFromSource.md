...
Для нового консольного ядра нужен Rust `1.89` или новее:
...
| `--dc-ip` | `2:149.154.167.220`, `4:149.154.167.220` | Целевой IP для DC (параметр можно указывать несколько раз). `--dc-ip` без значения отключает все прямые подключения к DC |
| `--no-cfproxy` | `false` | Отключить попытку [проксирования через Cloudflare](./CfProxy.md) |
| `--no-secure` | `false` | Подключаться к CF-proxy и CF-worker по обычному WebSocket на порту 80 вместо TLS |
