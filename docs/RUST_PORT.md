...
> реальных окружениях и считается тестовой. Нужны проверки на Windows, macOS и
> Linux, включая direct WebSocket, Worker/CfProxy, TCP fallback, Fake TLS и
> tray.
...
с GUI-настройками, auto-update и autostart. Rust binary не должен
автоматически обновляться бинарниками из upstream Python-релизов.
...
открываются в системном JSON-редакторе; встроенного GUI-редактора и updater в
Rust frontend нет. Обычный Restart перечитывает proxy-параметры и язык, но
изменения `verbose` и `log_max_mb` применяются только после полного перезапуска
desktop process.
...
- Список Worker-доменов выбирается локально для сессии и не перемешивается
  конкурентно всеми задачами.
...
- Rust tray покрывает основные runtime-действия, но полный GUI-редактор,
  auto-update, Windows autostart/portable mode и sleep/resume интеграция пока
  остаются legacy-возможностями.
