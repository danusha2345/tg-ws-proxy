...
Версия **1.11.1** исправляет восстановление Windows-окна из трея после
скрытия и минимизации: Windows API сначала возвращает окно, затем возобновляется
отрисовка egui. Нативный регрессионный тест проверяет повторное скрытие,
минимизацию и сохранение развёрнутого состояния без цикла отрисовки.
Полная проверка интерфейса на реальном Windows-компьютере остаётся отдельным
этапом. Прокси-ядро и формат настроек не изменены.
