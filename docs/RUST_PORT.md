...
находиться среди скрытых значков. Встроенного GUI-редактора и updater в Rust
frontend нет. Обычный Restart перечитывает proxy-параметры и язык, но изменения
