...
Версия **1.12.0** переносит изменения upstream 1.10.3–1.10.4: флаг
`--no-secure` (CF-proxy и Worker по обычному WebSocket на порту 80),
`--dc-ip` без значения для отключения прямых подключений и системные корневые
сертификаты в дополнение к встроенным. Updater при недоступности GitHub
использует зеркало GitLab с той же проверкой SHA-256.

Версия 1.11.2 маскирует имена сторонних доменов в журнале, как upstream
1.10.2, получает новую иконку и обновлённые зависимости: Rust 1.98 и
reqwest 0.13 для updater'а с прежним провайдером ring. Версия 1.11.1 исправила
восстановление Windows-окна из трея после скрытия и минимизации; нативный
регрессионный тест проверяет это без цикла отрисовки. Полная проверка
интерфейса на реальном Windows-компьютере остаётся отдельным этапом.
Прокси-протокол и формат настроек не изменены.
...
## Синхронизация с upstream 1.10.2

- Имена сторонних доменов в консоли и в файле журнала маскируются, как в
  upstream: у каждой метки, кроме TLD, остаётся первая половина символов,
  остальные заменяются `*`. Домены `telegram.org`, IP-адреса, версии и имена
  `.log` не изменяются.
- Попытка взять готовое соединение из пула при таймауте IP, дозаполнение пула
  после ротации и стартовая задержка backoff в 1 секунду уже были реализованы
  в 1.11.0 и совпадают с upstream.
